...
<figure>
  <figcaption><strong>Figure 1: Employment Insurance benefit claim cycle</strong></figcaption>
  <img class="img-responsive" src="/dam/medicare/benefit-cycle.png" alt="Overview workflow diagram illustrating the multi-stage claim evaluation process">
  <details>
    <summary>Figure 1: Employment Insurance benefit claim cycle - Text Equivalent</summary>
    <p>The processing cycle involves three main stages: initial application verification, employer Record of Employment matching, and bi-weekly report validation.</p>
  </details>
</figure>
...
<figure>
  <figcaption><strong>Figure 1&nbsp;: Cycle de traitement des demandes de prestations d'assurance-emploi</strong></figcaption>
  <img class="img-responsive" src="/dam/medicare/benefit-cycle.png" alt="Diagramme de flux des opérations illustrant le processus d'évaluation des demandes en plusieurs étapes">
  <details>
    <summary>Figure 1&nbsp;: Cycle de traitement des demandes de prestations d'assurance-emploi - Équivalent textuel</summary>
    <p>Le cycle de traitement comprend trois étapes principales&nbsp;: la vérification initiale de la demande, le jumelage du relevé d'emploi de l'employeur et la validation du rapport bimensuel.</p>
  </details>
</figure>
...
function isAutoGeneratedImageAlt(text) {
  ...
  // Common Microsoft Word / Office AI auto-generated alt text patterns
  if (
    norm.includes('description generee automatiquement') ||
    norm.includes('description automatiquement generee') ||
    norm.includes('description automatically generated') ||
    norm.includes('texte genere automatiquement') ||
    norm.includes('image generee automatiquement') ||
    norm.includes('objet genere automatiquement') ||
    norm.includes('icone generee automatiquement') ||
    norm.includes('description generee par') ||
    norm.includes('description generated with')
  ) {
    return true;
  }

  // Word auto-generated shape / graphic alt text (e.g. alert box icons or banner bars)
  if (
    /^un objet (?:rectangulaire|carre|circulaire|de forme|graphique)/i.test(norm) ||
    /^une forme (?:rectangulaire|carree|circulaire|geometrique)/i.test(norm) ||
    /^une image contenant (?:un objet|du texte|une icone|un logo)/i.test(norm) ||
    /^a (?:rectangular|square|circular) object/i.test(norm) ||
    /^a picture containing (?:an object|text|an icon|a logo)/i.test(norm) ||
    /^a screenshot of/i.test(norm)
  ) {
    return true;
  }
...
function isRedundantFigureImageAlt(el) {
  if (!el || el.tagName?.toLowerCase() !== 'img') return false;
  const fig = el.closest('figure');
  if (!fig) return false;
  const hasFigcaption = !!fig.querySelector('figcaption');
  if (!hasFigcaption) return false;
  const alt = (el.getAttribute('alt') || '').trim();
  if (!alt) return true;
  // If alt is just a short label / reference to the figure like "Figure 1", "Figure 1.", "Figure A", "Image", etc.
  if (/^(?:Figure|Tableau|Table|Image|Graphique|Chart|Schéma)\s*[a-zA-Z0-9_-]*[.:]?$/i.test(alt)) {
    return true;
  }
  return false;
}

function isDecorativeOrAutoGeneratedElement(el) {
  if (!el || !el.tagName) return false;
  const tag = el.tagName.toLowerCase();
  if (tag === 'img') {
    const alt = el.getAttribute('alt') || '';
    if (!alt || isAutoGeneratedImageAlt(alt) || isRedundantFigureImageAlt(el)) return true;
  }
  return false;
}
...
function isLeafBlock(el) {
  ...
  if (tagName === 'img') {
    const alt = el.getAttribute('alt') || '';
    if (isRedundantFigureImageAlt(el)) return false;
    return !isAutoGeneratedImageAlt(alt);
  }
  ...
  if (tagName === 'li') {
    ...
    const nonListChildBlocks = Array.from(el.querySelectorAll(BLOCK_SELECTOR)).filter((child) => {
      ...
      if (isFootnoteBoilerplateElement(child) || isDecorativeOrAutoGeneratedElement(child)) return false;
  ...
  // If it contains child blocks, check if those child blocks are actual content blocks (not just boilerplate or decorative images)
  const childBlocks = Array.from(el.querySelectorAll(BLOCK_SELECTOR)).filter(
    (child) => child !== el && !isFootnoteBoilerplateElement(child) && !isDecorativeOrAutoGeneratedElement(child)
  );
...
function isTextEquivalentSummaryText(text) {
  if (!text) return false;
  const t = text.trim();
...
function isTextEquivalentSummaryBlock(block) {
  if (!block) return false;
  if (block.tag === 'summary') return true;
  return isTextEquivalentSummaryText(block.text);
}

function isFigcaptionBlock(block) {
  if (!block) return false;
  ...
  return (
    /^(?:Figure|Tableau|Table|Graphique|Chart|Schéma|Diagram)\s+[a-zA-Z0-9_-]+\s*[\-–—:：.]/i.test(t) ||
    /^(?:Figure|Tableau|Table|Graphique|Chart|Schéma|Diagram)\s+[a-zA-Z0-9_-]+(?:\s+|$)/i.test(t)
  );
}
...
function extractBlocks(rootEl) {
  ...
  return all
    ...
    .filter((b) => !isAutoGeneratedImageAlt(b.text));
}
...
function extractSectionIdentifier(text, tag = '') {
  if (!text) return null;
  const t = text.trim();
  ...
  if (enBlock && frBlock) {
    ...
    if (enIsFigcaption && frIsFigcaption) {
      ...
      if (enSecId && frSecId && enSecId === frSecId) {
        return 20.0; // Deterministic anchor lock for figure caption matching
      }
      return 14.0;
    }
    if (enIsFigcaption !== frIsFigcaption) {
    ...
    // Structural section identifier match (e.g. "Appendix 2" vs "Annexe 2", "Table 1" vs "Tableau 1", "Figure 1" vs "Figure 1")
    const enSecId = extractSectionIdentifier(enBlock.text, enTag);
    const frSecId = extractSectionIdentifier(frBlock.text, frTag);
    if (enSecId && frSecId) {
      if (enSecId === frSecId) {
...
function parseFrDocxHtml(rawDocxHtml, filename = 'Uploaded Document.docx') {
  ...
  // Clean auto-generated decorative shapes, alert box screenshots, and AI image artifacts from Word
  doc.body.querySelectorAll('img').forEach((img) => {
    const alt = img.getAttribute('alt') || '';
    if (!alt || isAutoGeneratedImageAlt(alt)) {
  ...
  state.frRawDocxHtml = doc.body.innerHTML;
...
function computeAlignment(targetBlockIndex = null) {
  ...
  for (let idx = 0; idx < rawRows.length; idx++) {
    ...
    if (row.enIndex !== null && row.frIndex !== null && !row.skip) {
      const enB = state.enBlocks[row.enIndex];
      const enBrCount = enB ? (enB.brCount || (enB.text.includes('\n') ? enB.text.split('\n').length - 1 : 0)) : 0;
      
      if (enBrCount > 0) {
        const frIndices = [row.frIndex];
        let nextIdx = idx + 1;
        let gathered = 0;
        
        while (nextIdx < rawRows.length && gathered < enBrCount) {
          const nextRow = rawRows[nextIdx];
          if (nextRow.enIndex === null && nextRow.frIndex !== null && !nextRow.skip) {
            const nextFrB = state.frBlocks[nextRow.frIndex];
            const isCompatible = nextFrB && ['p', 'li', 'td', 'th', 'div', 'blockquote'].includes(nextFrB.tag);
            if (isCompatible) {
              frIndices.push(nextRow.frIndex);
              gathered++;
              nextIdx++;
              continue;
            }
        ...
        if (frIndices.length > 1) {
          row.groupedFrIndices = frIndices;
          row.mergedFrText = frIndices.map((fi) => state.frBlocks[fi] ? state.frBlocks[fi].text : '').join('\n');
          row.mergedFrSpans = frIndices.flatMap((fi) => (state.frBlocks[fi] && state.frBlocks[fi].spans) || []);
          idx = nextIdx - 1; // Advance idx past the grouped rows
        }
      }
    }
    rows.push(row);
